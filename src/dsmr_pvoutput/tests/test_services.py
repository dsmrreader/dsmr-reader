...
class TestServices(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.schedule_process.update(active=True, planned=timezone.make_aware(datetime.datetime(2000, 1, 1)))
    ...
    def test_get_next_export(self, now_mock):
        ...
        now_mock.return_value = timezone.make_aware(datetime.datetime(2018, 2, 1, hour=12, minute=13, second=15))
        ...
        now_mock.return_value = timezone.make_aware(datetime.datetime(2018, 2, 1, hour=12, minute=25, second=50))
        result = dsmr_pvoutput.services.get_next_export()
        self.assertEqual(result.hour, 12)
        ...
        # Pass hour mark.
        now_mock.return_value = timezone.make_aware(datetime.datetime(2018, 2, 1, hour=12, minute=59, second=30))
        result = dsmr_pvoutput.services.get_next_export()
    ...
    def test_get_export_data(self, now_mock):
        ...
        now_mock.return_value = timezone.make_aware(datetime.datetime(2017, 9, 30, hour=15))
        ...
        # First sync, next day.
        now_mock.return_value = timezone.make_aware(datetime.datetime(2017, 10, 1, hour=13))  # Include 2 EC's.
        ...
        now_mock.return_value = timezone.make_aware(datetime.datetime(2017, 10, 1, hour=15))  # Include all 3 EC's.
        result = dsmr_pvoutput.services.get_export_data(next_export=None, upload_delay=0)
        ...
        now_mock.return_value = now_mock.return_value + datetime.timedelta(minutes=1)
        dsmr_pvoutput.services.get_export_data(next_export=timezone.now(), upload_delay=1)
    ...
    def test_export_no_electricity(self, now_mock, requests_mock):
        ...
        now_mock.return_value = timezone.make_aware(datetime.datetime(2017, 10, 1, hour=15))
        self._apply_fake_settings()
    ...
    def test_export_postponed(self, send_robust_mock, now_mock, export_data_mock, requests_post_mock):
        ...
        now_mock.return_value = timezone.make_aware(datetime.datetime(2017, 10, 1, hour=1))
