...
class TestDsmrSuperuserCommand(InterceptCommandStdoutMixin, TestCase):
    ...
    def test_missing_username_raises(self):
        with mock.patch.dict(os.environ, {"DSMRREADER_ADMIN_USER": "", "DSMRREADER_ADMIN_PASSWORD": "secret"}):
            with self.assertRaises(CommandError):
                self._intercept_command_stdout("dsmr_superuser")

    def test_missing_password_raises(self):
        with mock.patch.dict(os.environ, {"DSMRREADER_ADMIN_USER": "admin", "DSMRREADER_ADMIN_PASSWORD": ""}):
            with self.assertRaises(CommandError):
                self._intercept_command_stdout("dsmr_superuser")
    ...
    def test_creates_new_superuser(self):
        ...
        with mock.patch.dict(os.environ, {"DSMRREADER_ADMIN_USER": "admin", "DSMRREADER_ADMIN_PASSWORD": "secret"}):
            self._intercept_command_stdout("dsmr_superuser")
    ...
    def test_updates_existing_superuser_password(self):
        ...
        with mock.patch.dict(os.environ, {"DSMRREADER_ADMIN_USER": "admin", "DSMRREADER_ADMIN_PASSWORD": "new-secret"}):
            self._intercept_command_stdout("dsmr_superuser")
