...
class TestUpdateChecker(InterceptCommandStdoutMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.schedule_process.update(
            active=True, planned=timezone.make_aware(timezone.datetime(2010, 1, 1))
        )
    ...
    def test_no_update(self, now_mock, is_latest_version_mock):
        ...
        self.assertEqual(
            self.schedule_process.planned, timezone.now() + timezone.timedelta(days=7)
        )
    ...
    @mock.patch("django.utils.timezone.now")
    def test_has_update(
        self, now_mock, display_dashboard_message_mock, is_latest_version_mock
    ):
        now_mock.return_value = timezone.make_aware(timezone.datetime(2010, 1, 1))
