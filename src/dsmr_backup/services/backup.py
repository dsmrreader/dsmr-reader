...
def run(scheduled_process: ScheduledProcess) -> None:
    ...
    next_backup_timestamp = timezone.now() + timedelta(days=backup_settings.backup_interval_in_days)
...
def on_backup_failed(process_handle) -> NoReturn:
    ...
    logger.critical(
        " - Unexpected exit code (%s) for backup: %s",
        process_handle.returncode,
        error_message,
    )

    dsmr_frontend.services.display_dashboard_message(
        message=_("Backup creation failed, please check the dsmr_backend logfile.")
    )
