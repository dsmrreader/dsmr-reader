...
class DsmrReadingFilter(filters.FilterSet):
    ...
    timestamp__gte = filters.DateTimeFilter(
        field_name=FIELD, lookup_expr='gte', label='Reading timestamp must be after or equal to `X`'
    )
    timestamp__lte = filters.DateTimeFilter(
        field_name=FIELD, lookup_expr='lte', label='Reading timestamp must be before or equal to `X`'
    )
...
class EnergySupplierPriceFilter(filters.FilterSet):
    start__gte = filters.DateFilter(
        field_name='start', lookup_expr='gte', label='Contract start date must be after or equal to `X`'
    )
    start__lte = filters.DateFilter(
        field_name='start', lookup_expr='lte', label='Contract start date must be before or equal to `X`'
    )
    end__gte = filters.DateFilter(
        field_name='end', lookup_expr='gte', label='Contract end date must be after or equal to `X`'
    )
    end__lte = filters.DateFilter(
        field_name='end', lookup_expr='lte', label='Contract end date must be before or equal to `X`'
    )
...
class ElectricityConsumptionFilter(filters.FilterSet):
    ...
    read_at__gte = filters.DateTimeFilter(
        field_name=FIELD, lookup_expr='gte', label='Consumption timestamp must be after or equal to `X`'
    )
    read_at__lte = filters.DateTimeFilter(
        field_name=FIELD, lookup_expr='lte', label='Consumption timestamp must be before or equal to `X`'
    )
...
class HourStatisticsFilter(filters.FilterSet):
    ...
    hour_start__gte = filters.DateTimeFilter(
        field_name=FIELD, lookup_expr='gte', label='Hour start must be after or equal to `X`'
    )
    hour_start__lte = filters.DateTimeFilter(
        field_name=FIELD, lookup_expr='lte', label='Hour start must be before or equal to `X`'
    )
