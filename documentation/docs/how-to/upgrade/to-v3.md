...
> Note: If you're using Docker, you can probably just install the `v3.x` version of the Docker container without any of the steps below.
...
> **Warning:** Only execute this section if you're running DSMR-reader with `Python 3.5` or lower!
...
Try running `python3.6 --version` to see if things worked out. If you're getting any errors, do not continue with the upgrade.

The Python upgrade continues later, after creating a backup.

## 4. Backup

Create a fresh database backup and store it somewhere safe.
...
Make sure the file is of some (reasonable) size, e.g.:
