...
> Note: If you're using Docker, you can probably just install the `v4.x` version of the Docker container without having to perform any of the steps below.
...
> **Danger:** See the changelog for `v4.x releases` and higher. Check them before updating!
...
> Warning: If you've installed Python `3.6` or higher manually and the default Python version is below `3.6`, make sure to specify it in the `virtualenv` command above.
>
> Example:
>
> ```shell title="shell"
...
DSMR-reader started with a `settings.py` for your local settings. This has some disadvantages, especially regarding today's industry standards and how Docker works as well.
...
Now check the settings you were using in `dsmrreader/settings.py.BACKUP`. Compare them with the defaults in `.env`. If you find any differences (e.g. different database credentials or host), update the `.env` file accordingly. The format should be straightforward.
...
However, you can still use any custom Django settings in the `dsmrreader/settings.py`. For example: `X_FRAME_OPTIONS` or `USE_X_FORWARDED_HOST`.
...
The `dsmr_mqtt` process has been merged into `dsmr_backend`.
...
You should not see `dsmr_mqtt` anymore. Also, the other processes should be running as well again.
