...
class TestServices(InterceptCommandStdoutMixin, TestCase):
    ...
    @mock.patch("dropbox.Dropbox.files_get_metadata")
    def test_sync_content_not_modified(
        self,
    ...
    @mock.patch("dropbox.Dropbox.files_get_metadata")
    @mock.patch("os.stat")
    def test_sync_non_existing_remote_file(
        self,
        stat_mock,
        files_get_metadata_mock,
        upload_chunked_mock,
        ...
        files_get_metadata_mock.side_effect = dropbox.exceptions.ApiError(
            "12345", "ApiError('xxx', ApiError('not_found', None))", "x", "y"
        )
        ...
        self.schedule_process.delay(seconds=-1)
        files_get_metadata_mock.side_effect = dropbox.exceptions.ApiError(
            "67890", "ApiError('xxx', ApiError('other_error', None))", "x", "y"
        )
