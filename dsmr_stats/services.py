...
def analyze():
    ...
    if not is_data_available():
        return logging.debug('Stats: no data available')
    ...
    if target_day >= now.date():
        return logging.debug('Stats: Day should be in the past: %s', target_day)
    ...
    if unprocessed_readings:
        return logging.debug('Stats: Unprocessed readings found for %s', target_day)
    ...
    if not consumption_found:
        return logging.debug('Stats: No consumption found for %s', target_day)
    ...
    if dsmr_backend.services.backend.get_capabilities(capability='gas') and \
        ...
        return logging.debug('Waiting for first gas reading on the next day...')
